...
<xsl:template match="m:merge" >
   ...
   <xsl:message>
      <xsl:text>Compare XML log Details: </xsl:text>
   </xsl:message>
...
-->
<xsl:param name="with">base.xml</xsl:param>

<xsl:template match="*">

   <!--xsl:message>
      <xsl:text>The following tags added with base XML: </xsl:text>
   </xsl:message-->
...
<xsl:template name="m:merge">
   ...
   <xsl:choose>
	...
	<!--xsl:variable name="VariableName" select="$nodes1/@name"/-->

	 <!--xsl:if test="count($nodes2)=0 and $nodes1!=''">
     	        <xsl:message><xsl:text>child tags - </xsl:text>
		   <xsl:value-of select="$nodes1/@name"/>
	      </xsl:message>
          </xsl:if-->
